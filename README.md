<div align="center">

# 👋 Hi, I'm Vishal Sharma

### 🚀 Backend Software Engineer | Java | Spring Boot | Distributed Systems

<p>
  <a href="https://github.com/sharmavishal2">
    <img src="https://img.shields.io/badge/GitHub-sharmavishal2-181717?style=for-the-badge&logo=github" />
  </a>
  <a href="https://www.linkedin.com/in/sharma-vishal-1abc">
    <img src="https://img.shields.io/badge/LinkedIn-Vishal%20Sharma-0A66C2?style=for-the-badge&logo=linkedin" />
  </a>
  <a href="https://leetcode.com/u/sharma_vishal_1/">
    <img src="https://img.shields.io/badge/LeetCode-300%2B%20Problems-FFA116?style=for-the-badge&logo=leetcode" />
  </a>
  <a href="https://codolio.com/profile/sharma_vishal_1">
    <img src="https://img.shields.io/badge/Codolio-Profile-6C63FF?style=for-the-badge" />
  </a>
</p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,100:2563eb&height=180&section=header&text=Building%20Scalable%20Backend%20Systems&fontSize=32&fontColor=ffffff&animation=fadeIn&fontAlignY=38"/>

<p>
  <img src="https://komarev.com/ghpvc/?username=sharmavishal2&style=for-the-badge&color=2563eb&label=PROFILE+VIEWS" />
  <img src="https://img.shields.io/github/stars/sharmavishal2?style=for-the-badge&color=2563eb&label=TOTAL+STARS" />
  <img src="https://img.shields.io/github/followers/sharmavishal2?style=for-the-badge&color=2563eb&label=FOLLOWERS" />
</p>

</div>

---

## 👨‍💻 About Me

<img align="right" width="300" src="https://github-readme-stats.vercel.app/api?username=sharmavishal2&show_icons=true&theme=tokyonight&hide_border=true&count_private=false&include_all_commits=false" />

* 🚀 Backend-focused Software Engineer
* ☕ Building scalable applications with **Java & Spring Boot**
* ⚙️ Interested in **Distributed Systems & System Design**
* ⚡ Working with **Redis, PostgreSQL, REST APIs & Docker**
* 🧠 Solved **300+ DSA problems**
* 🏆 **AIR 102 — NEC 2025**
* 🏆 **CodeChef Global Rank 48 — PLACEPREP03**
* 👨‍💻 Team Leader — **Smart India Hackathon**
* 🌱 Currently strengthening **System Design, Distributed Systems & Cloud**

<br clear="right"/>

---

# 🛠️ Technical Skills

### 💻 Languages

<p>
  <img src="https://skillicons.dev/icons?i=java,python,js,sql" />
</p>

### ⚙️ Backend & Frameworks

<p>
  <img src="https://skillicons.dev/icons?i=spring,hibernate" />
  <img src="https://img.shields.io/badge/REST%20APIs-02569B?style=for-the-badge&logo=fastapi&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white" />
</p>

### 🗄️ Databases & Caching

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,redis" />
</p>

### 🎨 Frontend

<p>
  <img src="https://skillicons.dev/icons?i=react,html,css,tailwind" />
</p>

### ☁️ DevOps & Tools

<p>
  <img src="https://skillicons.dev/icons?i=docker,git,github,maven,postman" />
</p>

### 🤖 AI & APIs

<p>
  <img src="https://img.shields.io/badge/Generative%20AI-412991?style=for-the-badge" />
  <img src="https://img.shields.io/badge/LLM%20Integration-FF6F00?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Prompt%20Engineering-7C3AED?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Groq-000000?style=for-the-badge" />
</p>

---

# 🚀 Featured Projects

<p align="center">
  <i>Selected projects focused on backend engineering, scalability and real-world systems.</i>
</p>

---

## 🔥 Distributed Rate Limiter

> Production-oriented distributed rate limiting service built with **Spring Boot, Redis, PostgreSQL and Docker** for controlling API traffic under high concurrency.

### ✨ Highlights

* 🚦 Token Bucket, Sliding Window & Fixed Window algorithms
* ⚡ Redis atomic operations & Lua scripting
* 🔐 Secure REST APIs for rate-limit rule management
* 🗄️ PostgreSQL configuration persistence
* 🐳 Dockerized distributed infrastructure
* 📊 Request monitoring & rate-limit metrics
* ⚡ Sub-5ms evaluation latency

**Tech Stack:** `Java` `Spring Boot` `Redis` `PostgreSQL` `Docker` `JPA`

<p>
  <a href="https://github.com/sharmavishal2/Distributed-Rate-Limiter">
    <img src="https://img.shields.io/badge/VIEW%20PROJECT-181717?style=for-the-badge&logo=github" />
  </a>
</p>

---

## 🎯 GoalFlow

> Enterprise-style goal management platform supporting **Employee, Manager and Admin** workflows with approvals, reviews, AI suggestions and analytics.

### ✨ Highlights

* 👥 Role-Based Access Control
* 🔐 JWT authentication & authorization
* 🎯 Goal creation, approval & review cycles
* 🤖 Groq-powered AI goal suggestions
* 📊 Progress tracking & analytics dashboards
* ⚙️ Secure Spring Boot REST APIs

**Tech Stack:** `Java` `Spring Boot` `React` `PostgreSQL` `JWT` `Groq API`

<p>
  <a href="https://github.com/sharmavishal2/GoalFlow">
    <img src="https://img.shields.io/badge/REPOSITORY-181717?style=for-the-badge&logo=github" />
  </a>
  <a href="https://goal-flows.vercel.app/">
    <img src="https://img.shields.io/badge/LIVE%20DEMO-2563EB?style=for-the-badge&logo=vercel" />
  </a>
</p>

---

## 🛒 E-Commerce Backend

> Robust e-commerce REST API built with **Spring Boot** supporting authentication, product management, cart operations and order workflows.

### ✨ Highlights

* 🛍️ Product & category management
* 🔐 JWT-based authentication & authorization
* 🛒 Cart & order management
* 🔎 Paginated search & dynamic filtering
* 🗄️ PostgreSQL database
* ⚙️ Modular REST API architecture
* 🔒 Role-based access control

**Tech Stack:** `Java` `Spring Boot` `PostgreSQL` `Spring Security` `JWT` `JPA/Hibernate`

<p>
  <a href="https://github.com/sharmavishal2/E-Commerce-Application-by-Java-Spring-Boot">
    <img src="https://img.shields.io/badge/VIEW%20PROJECT-181717?style=for-the-badge&logo=github" />
  </a>
</p>

---

# 🤖 Other Notable Projects

<p align="center">

<a href="https://github.com/sharmavishal2/AlgoPush">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=sharmavishal2&repo=AlgoPush&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/sharmavishal2/MLH-HACKATHON---SmartHire">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=sharmavishal2&repo=MLH-HACKATHON---SmartHire&theme=tokyonight&hide_border=true" />
</a>

</p>

<p align="center">

<a href="https://github.com/sharmavishal2/Portfolio">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=sharmavishal2&repo=Portfolio&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/sharmavishal2/Quiz-Application">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=sharmavishal2&repo=Quiz-Application&theme=tokyonight&hide_border=true" />
</a>

</p>

---

# 📊 GitHub Analytics

<div align="center">

<img height="180" src="https://github-readme-stats.vercel.app/api?username=sharmavishal2&show_icons=true&theme=tokyonight&hide_border=true&count_private=false&include_all_commits=false" />

<img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=sharmavishal2&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" />

</div>

---

## 🔥 Contribution Streak

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=sharmavishal2&theme=tokyonight&hide_border=true" />

</div>

---

## 📈 Contribution Activity

<div align="center">

<img width="95%" src="https://github-readme-activity-graph.vercel.app/graph?username=sharmavishal2&theme=tokyo-night&hide_border=true&area=true&custom_title=Vishal's%20Contribution%20Activity" />

</div>

---

## 🗓️ Contribution Calendar

<div align="center">

<img src="https://ghchart.rshah.org/2563eb/sharmavishal2" alt="Vishal's GitHub Contribution Calendar" />

</div>

---

# 🏆 GitHub Achievements

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=sharmavishal2&theme=tokyonight&no-frame=true&no-bg=true&margin-w=8&row=2&column=6" />

</div>

---

# 🏅 Achievements

| 🏆 Achievement                    | 📌 Result                               |
| --------------------------------- | --------------------------------------- |
| 🇮🇳 NEC 2025                     | **AIR 102** among 4,000+ colleges       |
| 🥇 CodeChef — PLACEPREP03         | **Global Rank 48**                      |
| 🏆 CodeChef — Starter Contest 237 | **Global Rank 824**                     |
| 👨‍💻 Smart India Hackathon       | **Team Leader**                         |
| 🧠 LeetCode                       | **300+ DSA Problems**                   |
| 💼 JP Morgan Chase                | Software Engineering Virtual Experience |

---

# 🧩 Coding Profiles

<div align="center">

<a href="https://leetcode.com/u/sharma_vishal_1/">
<img src="https://img.shields.io/badge/LeetCode-300%2B%20Problems-FFA116?style=for-the-badge&logo=leetcode" />
</a>

<a href="https://codolio.com/profile/sharma_vishal_1">
<img src="https://img.shields.io/badge/Codolio-View%20Profile-6C63FF?style=for-the-badge" />
</a>

<a href="https://github.com/sharmavishal2">
<img src="https://img.shields.io/badge/GitHub-Profile-181717?style=for-the-badge&logo=github" />
</a>

</div>

---

# 🌱 Currently Learning

<div align="center">

`System Design` → `Distributed Systems` → `Redis` → `Kafka` → `Microservices` → `Cloud`

</div>

---

# 💬 Developer Philosophy

<div align="center">

### "Build systems that scale. Write code that lasts. Keep learning."

</div>

---

# 🌐 Connect With Me

<div align="center">

<a href="mailto:sharma2005vishal@gmail.com">
<img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail" />
</a>

<a href="https://www.linkedin.com/in/sharma-vishal-1abc">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin" />
</a>

<a href="https://sharmavishal2.github.io/Portfolio/">
<img src="https://img.shields.io/badge/Portfolio-Visit-2563EB?style=for-the-badge&logo=googlechrome" />
</a>

</div>

---

<div align="center">

### 🚀 Building scalable systems. Solving real-world problems.

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2563eb,100:0f172a&height=100&section=footer" />

</div>
